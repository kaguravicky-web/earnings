...
> 维护说明（2026-08-06）：本文保留了早期 GS/IBM 研究中实际遇到的数据问题，供后续 AI 避坑。当前统一优先级为：公司 IR/SEC > 正式业绩会 > 权威财经媒体 > 管理层采访 > X 与第三方数据。涉及近期数据时必须重新联网核验，不能把本文列出的历史网站或数字当成当前事实。
...
- **用途：** 捕捉实时市场叙事、行业争议、管理层/记者/分析师的增量观点和市场关注度
- **优先来源：** 公司官方账号、管理层、行业专家、可信分析师、长期覆盖该行业的记者
- **核验要求：** X只作为叙事和线索来源；财务数字、订单、客户和管理层引语应回到公告、SEC文件、业绩会或原始采访核验
- **记录字段：** 作者、账号、发布日期、原帖链接、一句话摘要、事实/观点/传闻分类
- **常见失败：** 帖子被删除、需要登录、上下文不完整、截图缺少原始链接、转帖误传
- **冲突处理：** 原始披露 > 完整采访/业绩会 > X原帖 > 转帖和截图；单一高热度帖子不视为市场共识

## 13. 采访、视频与播客

- **输入方式：** 用户直接提供采访、YouTube、播客或其他视频链接
- **处理方式：** 优先读取官方字幕、文字稿、节目说明和可验证时间戳，提炼核心论点、证据、涉及公司和产业链
- **输出要求：** 标注与财报/业绩会的共识、分歧、当前叙事影响和证伪信号
- **常见失败：** 无字幕、地区限制、登录限制、只有音频无法可靠转写
- **失败处理：** 明确要求用户补充文字稿、字幕或关键截图；不得在未读取内容时声称已经完整总结
- **引用原则：** 保留原始链接和节目/受访者名称，避免长段复制受版权保护的原文
